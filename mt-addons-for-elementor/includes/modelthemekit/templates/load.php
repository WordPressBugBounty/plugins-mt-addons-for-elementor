...
use \ModelthemeKit\Templates\ModelthemeKit_Templates as Init;
use \Elementor\Core\Common\Modules\Ajax\Module as Ajax;
use \Elementor\Plugin;

if ( ! defined( 'ABSPATH' ) ) {exit;}

class ModelthemeKit_Load{
    ...
    public static function get_library() {
        if ( is_null( self::$library_data ) ) {
            self::$library_data = new ModelthemeKit_Api();
        }
        return self::$library_data;
    }
 
    public function register_ajax_actions( Ajax $ajax ) {
        $ajax->register_ajax_action( 'get_mtkit_library_data', function( $data ) {
