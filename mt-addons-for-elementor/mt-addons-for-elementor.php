 ...
 * 
 * 
 * @fs_premium_only /public/shortcodes/elementor/premium-widgets/, /includes/modelthemekit/
 * 
 * 
 */
...
if ( function_exists( 'mtfe_fs' ) ) {
    mtfe_fs()->set_basename( true, __FILE__ );
} else {
    ...
    if ( mtfe_fs()->is__premium_only() ) {
        ...
        DEFINE( 'MT_ADDONS_PREMIUM_PLUGIN_BASE', plugin_dir_path( __FILE__ ).'public/shortcodes/elementor/premium-widgets/');
        require_once('public/shortcodes/elementor/premium-widgets/include.php');
        require_once('includes/modelthemekit/modelthemekit.php');

        function mt_addons_premium_widget_categories( $elements_manager ) {
            $elements_manager->add_category(
            'mt-addons-premium',
                [
                    'title'   => esc_html__( 'MT Addons Premium', 'mt-addons' ),
                    'icon'    => 'fa fa-plug',
                ]
            );
        }
        add_action( 'elementor/elements/categories_registered', 'mt_addons_premium_widget_categories' );
    ...
    if (!function_exists('mt_addons_kses_allowed_html')) {
        ...
        add_filter( 'wp_kses_allowed_html', 'mt_addons_kses_allowed_html', 10, 2);
    }
}