            ...
            <div class="mtfe-inner">
                <div class="mtfe-tab-content elements">
                    <div class="mtfe-inner mtfe-box-inner">
                        ...
                        <?php foreach ($all_widgets as $key => $widgets_group) { ?>
                            <h2><?php echo esc_html($widgets_group['title']); ?></h2>
                            <div class="mtfe-elements-deactivate">
                                <?php unset($widgets_group['title']); ?>
                                <?php foreach ($widgets_group as $key => $widget) { ?>
                                    <div class="mtfe-row mtfe-type-checkbox <?php if($widget['premium'] == 'true'){echo $is_pro;}; ?>">
                                        <div>
                                            <label class="mtfe-label"><?php echo esc_html($widget['name']); ?><?php if($widget['premium'] == 'true'){ ?><span class="mtfe-premium-tag"><?php esc_html_e('PRO','mt-addons'); } ?></label>
                                            <div class="mtfe-spacer" style="height: 5px"></div>
                                            <p><a target="_blank" href="<?php echo esc_html(MT_ADDONS_LIVE_URL.$widget['presentation_url']); ?>"><?php esc_html_e('Live Preview', 'mt-addons'); ?></a></p>
                                        </div>
                                        <div class="mtfe-toggle">
                                            <input type="checkbox" class="mtfe-checkbox" name="mtfe_el_<?php echo esc_html($key); ?>"
                                                   id="mtfe_el_<?php echo esc_html($key); ?>" data-default=""
                                                   value="<?php echo esc_attr(mtfe_get_option( 'mtfe_el_'.$key, false )); ?>" 
                                                   <?php echo checked( !empty(mtfe_get_option( 'mtfe_el_'.$key, false )), 1, false ) ;?>>
                                            <label for="mtfe_el_<?php echo esc_html($key); ?>"></label>
                                        </div>
                                    </div>
                                <?php } ?>
                            </div>
